...
def main(args):
    ...
    # wandb.init(project="SE", name='SE  w>0.8 w<0.01', tags=["SE"])

    # wandb.init(project="GR", name='19|GR + w=4 groups=2 子网络loss平均', tags=["GR"])

    wandb.init(project="DEBUG", name="debug")
    # wandb.init(project="DVS",name="19|GR w=4 ds=0.1 bs=32")
    # wandb.init(project="DVS",name="19|baseline bs=32")
    ...
    if args.norm_list != None:
        if args.norm_list[-1] == "+":
            ...
            args.norm_list = arithmetic_series(
                float(args.norm_list[0]), float(args.norm_list[1]), args.T
            )
        elif args.norm_list[-1] == "*":
            assert len(args.norm_list) == 3
            args.norm_list = geometric_series(
                float(args.norm_list[0]), float(args.norm_list[1]), args.T
            )
    ...
    output_dir = os.path.join(
        "./logs", f'{time.strftime("%Y-%m-%d_%H:%M:%S")}_datasets_{args.dataset}'
    )
    utils.mkdir(output_dir)
    if args.redirect:
        outputs_dir = os.path.join(
            "./log", f'{time.strftime("%Y-%m-%d_%H:%M:%S")}_datasets_{args.dataset}.txt'
        )
    ...
    data_loader, data_loader_test = init_data(
        args.dataset + ("_aug" if args.aug else "")
    )(
    ...
    train_tb_writer = SummaryWriter(
        output_dir + "_logs/train", purge_step=purge_step_train
    )
    ...
    # define train class
    Train_Loss = init_train(args.loss_type)(
        model,
        args.device,
        lr_scheduler,
        optimizer,
...
def parse_args():
    ...
    parser.add_argument(
        "--data_path",
        default="/data/wupeixuan/CIFAR_100_SNN_Higher_Jelly/CIFAR100/",
        help="dataset",
        type=str,
    )
    ...
    parser.add_argument("--v_threshold", default=1.0, type=float)
