...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=myBatchNorm2d, single_step_neuron: callable = None, **kwargs):
        ...
        if 'LIFAct' in str(single_step_neuron):
            ...
            if self.downsample!=None:
                self.downsample_sn = single_step_neuron(planes)
        else:
...
class MultiStepBasicBlock(BasicBlock):
    ...
    def forward(self, x_seq):
        ...
        if self.downsample is not None:
            identity = self.downsample_sn(self.downsample[1](functional.seq_to_ann_forward(x_seq, self.downsample[0])))
