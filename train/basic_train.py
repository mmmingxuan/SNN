...
class train_1_loss:
    def __init__(
        ...
        self.cos_lr_T = (
            self.epochs
        )  # Commonly we set cos_lr_T as the same as total epochs
    ...
    def train_one_epoch(self, epoch, div=1):
        ...
        grad_normalized_accumulator = defaultdict(list)
        ...
        for image, target in metric_logger.log_every(
            self.data_loader, self.print_freq, header
        ):
            ...
            if self.scaler is not None:
                with amp.autocast():
                    ...
                    loss = torch.tensor([0.0], device=output[0].device)
            ...
            if self.scaler is not None:
                ...
                for name, param in self.model.named_parameters():
                    if "conv" in name and "weight" in name and param.grad is not None:
                        grad = param.grad.data / 100000
                        grad_norm = grad.norm(2).item()
                        grad_normalized = grad / param.data.numel()
                        grad_normalized_norm = grad_normalized.norm(2).item()

                        # 存储梯度数据
                        grad_accumulator[name].append(grad_norm)
                        grad_normalized_accumulator[name].append(grad_normalized_norm)
            ...
            #     for t in range(10):
            #         sn_list[index].norms[t].append(Node_instance.norms[t])
            ...
            metric_logger.update(
                loss=loss_s, lr=round(float(self.optimizer.param_groups[0]["lr"]), 6)
            )

            metric_logger.meters["acc1"].update(acc1_s, n=batch_size)
            metric_logger.meters["acc5"].update(acc5_s, n=batch_size)
            metric_logger.meters["img/s"].update(
                round(float(batch_size / (time.time() - start_time)), 5)
            )
            # self.lr_scheduler.step_update(num_updates=num_updates)

        # sn_log = {}
        # 保存到磁盘
        # torch.save(grad_accumulator, f"log_grad_16loss/grad_{epoch}.pth")
        # torch.save(
        #     grad_normalized_accumulator, f"log_grad_16loss/grad_normalized_{epoch}.pth"
        # )

        # 清空累积器以释放内存
        # grad_accumulator.clear()
        # grad_normalized_accumulator.clear()

        log = {}
        # import pandas as pd
        import numpy as np

        # log["mask1_max1"] = np.mean(self.model.mask1_max1)
        # log["mask1_max2"] = np.mean(self.model.mask1_max2)
        # log["mask1_max3"] = np.mean(self.model.mask1_max3)
        # log["mask2_max1"] = np.mean(self.model.mask2_max1)
        # log["mask2_max2"] = np.mean(self.model.mask2_max2)
        # log["mask2_max3"] = np.mean(self.model.mask2_max3)
        # log["mask3_max1"] = np.mean(self.model.mask3_max1)
        # log["mask3_max2"] = np.mean(self.model.mask3_max2)
        # log["mask3_max3"] = np.mean(self.model.mask3_max3)

        # log["mask1_q1"] = np.mean(self.model.mask1_q1)
        # log["mask1_q2"] = np.mean(self.model.mask1_q2)
        # log["mask1_q3"] = np.mean(self.model.mask1_q3)
        # log["mask2_q1"] = np.mean(self.model.mask2_q1)
        # log["mask2_q2"] = np.mean(self.model.mask2_q2)
        # log["mask2_q3"] = np.mean(self.model.mask2_q3)
        # log["mask3_q1"] = np.mean(self.model.mask3_q1)
        # log["mask3_q2"] = np.mean(self.model.mask3_q2)
        # log["mask3_q3"] = np.mean(self.model.mask3_q3)

        # log["mask1_activ"] = np.mean(self.model.mask1_activ)
        # log["mask1_backg"] = np.mean(self.model.mask1_backg)
        # log["mask2_activ"] = np.mean(self.model.mask2_activ)
        # log["mask2_backg"] = np.mean(self.model.mask2_backg)
        # log["mask3_activ"] = np.mean(self.model.mask3_activ)
        # log["mask3_backg"] = np.mean(self.model.mask3_backg)
        # log["mask1_Eras"] = np.mean(self.model.mask1_activ)
        # log["mask2_Eras"] = np.mean(self.model.mask2_activ)
        # log["mask3_Eras"] = np.mean(self.model.mask3_activ)
        # log["p0"] = np.mean(self.model.p0)
        # log["p1"] = np.mean(self.model.p1)
        # log["p2"] = np.mean(self.model.p2)
        # log["p3"] = np.mean(self.model.p3)
        ...
        #     Node_instance.avg()
        #     for i in range(len(Node_instance.avg_norm)):
        #         log[f"sn{index+1}_norm{i+1}"] = Node_instance.avg_norm[i]
    ...
    def evaluate_one_epoch(self):
        ...
        with torch.no_grad():
            for image, target in metric_logger.log_every(
                self.data_loader_test, self.print_freq, header
            ):
    ...
    def save_checkpoint(self, epoch):
        ...
        utils.save_on_master(
            checkpoint, os.path.join(self.output_dir, "checkpoint_latest.pth")
        )
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            train_loss, train_acc1, train_acc5, log = self.train_one_epoch(epoch)
            ...
            print(
                ...
                self.test_acc5_at_max_test_acc1,
            )
    ...
    def train_one_epoch_save_grad(self, epoch):
        ...
        for image, target in metric_logger.log_every(
            ...
            metric_logger.update(
                loss=loss_s, lr=round(float(self.optimizer.param_groups[0]["lr"]), 5)
            )
        ...
        save_path = os.path.join(
            self.output_dir + "/grad", f"epoch_{epoch}_gradient_sums.pt"
        )
    ...
    def save_norm_diff_train(self, epoch):
        with open(
            self.output_dir + f"/norm_diff/norm_diff_{epoch}_train.json", "w"
        ) as f:
            json.dump(self.diffsum_train, f)
        if os.path.exists(
            self.output_dir + f"/norm_diff/norm_diff_{epoch-1}_train.json"
        ):
            os.remove(self.output_dir + f"/norm_diff/norm_diff_{epoch-1}_train.json")

        torch.save(
            self.diffsum_train_tensor,
            self.output_dir + f"/norm_diff/norm_diff_{epoch}_train.pt",
        )
    ...
    def save_norm_diff_test(self, epoch):
        with open(
            self.output_dir + f"/norm_diff/norm_diff_{epoch}_test.json", "w"
        ) as f:
            json.dump(self.diffsum_test, f)
        if os.path.exists(
            self.output_dir + f"/norm_diff/norm_diff_{epoch-1}_test.json"
        ):
            os.remove(self.output_dir + f"/norm_diff/norm_diff_{epoch-1}_test.json")

        torch.save(
            self.diffsum_train_tensor,
            self.output_dir + f"/norm_diff/norm_diff_{epoch}_test.pt",
        )
    ...
    def print_temp_norm_list(self, module, parent_name="", reload=False, Train=True):
        ...
        for name, child in module.named_children():
            ...
            if hasattr(child, "temp_norm_list"):
                ...
                self.temp[current_name]["temp_norm_list"] = getattr(
            ...
            if hasattr(child, "grad_l_1_list"):
                ...
                if current_name not in self.temp:
                    self.temp[current_name] = {}
                if Train:
                    self.temp[current_name]["grad_l_1_list"] = [
                        [float(l.grad) for l in lists]
                        for lists in getattr(child, "grad_l_1_list")
                    ]
                if reload:
                    child.grad_l_1_list = []
            ...
            if hasattr(child, "grad_l_list"):
                ...
                if current_name not in self.temp:
                    self.temp[current_name] = {}
                if Train:
                    self.temp[current_name]["grad_l_list"] = [
                        [float(l.grad) for l in lists]
                        for lists in getattr(child, "grad_l_list")
                    ]
                if reload:
                    child.grad_l_list = []
            ...
            if hasattr(child, "delta_v"):
                ...
                if Train:
                    self.temp_tensor[current_name]["delta_v"] = getattr(
                        child, "delta_v"
                    )
                if reload:
                    child.delta_v = []
...
class train_16_loss(train_1_loss):
    def loss_calculate(self, output, target):
        ...
        if output.dim() == 3:
            # for i in range(len(output)):
            #     loss += self.loss_func(output[i], target)
            # return loss
            for i in range(len(output)):
                if i < len(output) - 4:
                    loss += self.loss_func(output[i], target)/2
                else:
                    loss += self.loss_func(output[i], target)
            return loss            
        else:
...
class train_16_loss_high_zhanbi_1(train_1_loss):
    ...
    def loss_calculate(self, output, target):
        loss = 0
        confidences = [
            [None for _ in range(len(output[0]))] for _ in range(len(output))
        ]
